    ...
    if (req.files && req.files.length > 0) {
        ...
        try {
            ...
            await Promise.all(req.files.map(async (file) => {
                ...
                const optimizedFiles = await OptimizeAvif(buffer);

                // Ensure optimizedFiles[0].data is a Buffer
                if (optimizedFiles.length > 0) {
                    const avifBuffer = Buffer.from(optimizedFiles[0].data);
                    archive.append(avifBuffer, { name: file.originalname.replace(/\.[^/.]+$/, ".avif") });
...
async function OptimizeAvif(srcBuffer) {
    const files = await imagemin.buffer(srcBuffer, {
        plugins: [
            imageminAvif({ quality: 75 }),
        ]
    });
